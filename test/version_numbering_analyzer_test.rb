...
#    assert_equal([], @vna.versionInconsistencies.jumpPairs[VersionNumber.versionCompoundMethods[:secondVersionCompound]])
  ...
  def test_cycle_lengths
    ...
    assert_equal([4], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:firstVersionCompound]))
    assert_equal([1,1,1,1], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:secondVersionCompound]))
    @vna = VersionNumberingAnalyzer.new(['1.0','1.1','1.2', '1.3', '2.0', '2.1'])
    assert_equal([4, 2], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:firstVersionCompound]))
    assert_equal([1,1,1,1,1,1], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:secondVersionCompound]))
    @vna = VersionNumberingAnalyzer.new(['1.0.0','1.0.1','1.0.2', '1.0.3', '2.0', '2.1', '2.2'])
    assert_equal([4, 3], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:firstVersionCompound]))
    assert_equal([4, 1, 1, 1], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:secondVersionCompound]))
    assert_equal([1,1,1,1,1,1,1], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:thirdVersionCompound]))
    assert_equal([1,1,1,1,1,1,1], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:fourthVersionCompound]))
    @vna = VersionNumberingAnalyzer.new(['1.0.0','1.0.1','1.0.3', '1.0.4', '2.0.1', '2.0.2'])
    assert_equal([4,2], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:firstVersionCompound]))
    assert_equal([4,2], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:secondVersionCompound]))
    assert_equal([1,1,1,1,1,1], @vna.getCycleLengths(VersionNumber.versionCompoundMethods[:thirdVersionCompound]))
